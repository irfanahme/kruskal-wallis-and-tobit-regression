...
library(deaR)
data <- read_data(df_clean, inputs = 3:5, outputs = 7)
View(data)
...
p <- ggplot(df_clean, aes(x = Passenger.Train.km..millions., fill=Number.of.Stations.Managed))
...
group_by(df_clean, Ã.Â.Â.Train.Operating.Company) %>%
  summarise(
    count = n(),
    mean = mean(Passenger.Train.km..millions., na.rm = TRUE),
    sd = sd(Passenger.Train.km..millions., na.rm = TRUE),
    median = median(Passenger.Train.km..millions., na.rm = TRUE),
    IQR = IQR(Passenger.Train.km..millions., na.rm = TRUE)
  )
kruskal.test(Passenger.Train.km..millions. ~Ã.Â.Â.Train.Operating.Company, data = df_clean)
cor(df_clean[, c("Number.of.Employees","Number.of.Stations.Managed","Route.km.operated", "Passenger.Train.km..millions.")])
# plot matrix

library(GGally)
ggpairs(df_clean[, c("Number.of.Employees", "Number.of.Stations.Managed", "Route.km.operated","Passenger.Train.km..millions.")])
# Tobit regression in give input columns 
library(VGAM)
summary(m <- vglm(Passenger.Train.km..millions. ~ Number.of.Employees + Number.of.Stations.Managed + Route.km.operated, tobit(Upper = 800), data = df_clean))
